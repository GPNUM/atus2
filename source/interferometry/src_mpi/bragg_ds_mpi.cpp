...
namespace MPI
{
  namespace RT_Solver_2
  {
    ...
    class Bragg_single  : public CRT_Base_IF_2_mpi<T,dim,4>
    {
    public:
    ...
    protected:
      ...
      using CRT_Base_2_mpi<T,dim,4>::m_map_stepfcts;
      using CRT_Base_2_mpi<T,dim,4>::m_fields;
      using CRT_Base_IF_2_mpi<T,dim,4>::DeltaL;
      using CRT_Base_IF_2_mpi<T,dim,4>::Amp;
      using CRT_Base_IF_2_mpi<T,dim,4>::Amp2;
      using CRT_Base_IF_2_mpi<T,dim,4>::laser_k;
      using CRT_Base_IF_2_mpi<T,dim,4>::laser_k2;
      using CRT_Base_IF_2_mpi<T,dim,4>::laser_dk;
      using CRT_Base_IF_2_mpi<T,dim,4>::laser_dk2;
      using CRT_Base_IF_2_mpi<T,dim,4>::laser_domh;
      using CRT_Base_IF_2_mpi<T,dim,4>::laser_domh2;
      using CRT_Base_IF_2_mpi<T,dim,4>::beta;
      using CRT_Base_IF_2_mpi<T,dim,4>::beta2;
      using CRT_Base_IF_2_mpi<T,dim,4>::phase;
      using CRT_Base_IF_2_mpi<T,dim,4>::phase2;
    };

    template<class T,int dim>
    Bragg_single<T,dim>::Bragg_single( ParameterHandler *p ) : CRT_Base_IF_2_mpi<T,dim,4>( p )
    {
      CPoint<dim> pt1;
      CPoint<dim> pt2;
    ...
    Psi.push_back(m_fields[i]->Getp2In());
      ...
      double re1, im1, tmp1, tmp2, tmp3, tmp4, V11, V22, V33, V44, Ep, Em, Omega, Omega2,phi[4];
      int mode1= 1, mode2 = 1;
      CPoint<1> x;
      ...
      #pragma omp parallel for private(x,re1,im1,tmp1,tmp2,tmp3,tmp4,V11,V22,V33,V44,Ep,Em,O11,O12,O21,O22,gamma_p,gamma_m,eta,Omega,Omega2,mode1,mode2,phi)
      for( int l=0; l<this->m_no_of_pts; l++ )
      {
        x = this->m_fields[0]->Get_x(l);
        ...
        Omega = mode1*Amp[0]*cos(laser_k[0]*x[0]-(laser_domh[0]+chirp_rate[0]*t1)*t1+phase[0]/2);
        Omega2 = mode2*Amp2[0]*cos(laser_k2[0]*x[0]-(laser_domh2[0]+chirp_rate2[0]*t1)*t1+phase2[0]/2);
        ...
        else if( Omega == 0.0 )
        {
          ...
          sincos( -0.5*(laser_dk2[0]*x[0]+phase2[0]), &im1, &re1 );
          eta[0] = re1;
          eta[1] = im1;
          ...
          tmp1 = 1.0/fabs((V11-Ep)*(V11-Ep)+Omega2*Omega2);
          gamma_p[0] = tmp1*re1;
          gamma_p[1] = tmp1*im1;

          sincos( -dt*Em, &im1, &re1 );
          tmp1 = 1.0/fabs((V11-Em)*(V11-Em)+Omega2*Omega2);
          gamma_m[0] = tmp1*re1;
          gamma_m[1] = tmp1*im1;
          ...
          O12[0] = -Omega2*(tmp1*gamma_p[0]+tmp2*gamma_m[0]);
          O12[1] = -Omega2*(tmp1*gamma_p[1]+tmp2*gamma_m[1]);
          ...
          Psi[2][l][0] = (O11[0]*gamma_p[0]-O11[1]*gamma_p[1]) + (O12[0]*gamma_m[0]-O12[1]*gamma_m[1]);
          Psi[2][l][1] = (O11[0]*gamma_p[1]+O11[1]*gamma_p[0]) + (O12[0]*gamma_m[1]+O12[1]*gamma_m[0]);

          Psi[3][l][0] = (O21[0]*gamma_p[0]-O21[1]*gamma_p[1]) + (O22[0]*gamma_m[0]-O22[1]*gamma_m[1]);
          Psi[3][l][1] = (O21[0]*gamma_p[1]+O21[1]*gamma_p[0]) + (O22[0]*gamma_m[1]+O22[1]*gamma_m[0]);
          continue;
        }
        else if( Omega2 == 0.0)
        {
          ...
          Psi[0][l][0] = (O11[0]*gamma_p[0]-O11[1]*gamma_p[1]) + (O12[0]*gamma_m[0]-O12[1]*gamma_m[1]);
          Psi[0][l][1] = (O11[0]*gamma_p[1]+O11[1]*gamma_p[0]) + (O12[0]*gamma_m[1]+O12[1]*gamma_m[0]);

          Psi[1][l][0] = (O21[0]*gamma_p[0]-O21[1]*gamma_p[1]) + (O22[0]*gamma_m[0]-O22[1]*gamma_m[1]);
          Psi[1][l][1] = (O21[0]*gamma_p[1]+O21[1]*gamma_p[0]) + (O22[0]*gamma_m[1]+O22[1]*gamma_m[0]);
          continue;
        }
...
int main( int argc, char *argv[] )
{
  ...
  try
  {
    if ( dim == 2 )
    {
      MPI::RT_Solver_2::Bragg_single<MPI::Fourier::cft_2d_MPI,2> rtsol( &params );
      rtsol.run_sequence();
    }
    else if ( dim == 3 )
    {
      MPI::RT_Solver_2::Bragg_single<MPI::Fourier::cft_3d_MPI,3> rtsol( &params );
      rtsol.run_sequence();
    }
    else
    {
