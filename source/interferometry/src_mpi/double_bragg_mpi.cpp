...
namespace MPI
{
  namespace RT_Solver
  {
    ...
    class Bragg_double : public CRT_Base_IF_mpi<T,dim,3>
    {
    public:
    ...
    protected:
      ...
      using CRT_Base_mpi<T,dim,3>::m_map_stepfcts;
      using CRT_Base_mpi<T,dim,3>::m_fields;
      using CRT_Base_mpi<T,dim,3>::m_header;
      using CRT_Base_mpi<T,dim,3>::m_gs;
      using CRT_Base_mpi<T,dim,3>::MTime;
      using CRT_Base_IF_mpi<T,dim,3>::Amp;
      using CRT_Base_IF_mpi<T,dim,3>::laser_k;
      using CRT_Base_IF_mpi<T,dim,3>::laser_dk;
      using CRT_Base_IF_mpi<T,dim,3>::chirp;
      using CRT_Base_IF_mpi<T,dim,3>::chirp_rate;
      using CRT_Base_IF_mpi<T,dim,3>::laser_domh;
      using CRT_Base_IF_mpi<T,dim,3>::beta;
      using CRT_Base_IF_mpi<T,dim,3>::phase;
      using CRT_Base_IF_mpi<T,dim,3>::DeltaL;
    };

    template<class T,int dim>
    Bragg_double<T,dim>::Bragg_double( ParameterHandler *p ) : CRT_Base_IF_mpi<T,dim,3>( p )
    {
    ...
    void Bragg_double<T,dim>::Do_Bragg_ad()
    {
      ...
      double re1, im1, tmp1, tmp2, tmp3, V11, V22, E1, E2, Omega_p, Omega_m;
      ptrdiff_t jk;
      ...
      for ( ptrdiff_t l=0; l<this->m_no_of_pts; l++ )
      {
        ...
        Omega_p = Amp[0]*cos(laser_k[0]*x[0]-(laser_domh[0]+chirp_rate[0]*t1)*t1-0.5*(phase[0]+m_Mirror[jk]));
        Omega_m = Amp[0]*cos(-laser_k[0]*x[0]-(laser_domh[0]-chirp_rate[0]*t1)*t1-0.5*(phase[0]-m_Mirror[jk]));
...
int main( int argc, char *argv[] )
{
  ...
  try
  {
    if ( dim == 2 )
    {
      MPI::RT_Solver::Bragg_double<MPI::Fourier::cft_2d_MPI,2> rtsol( &params );
      rtsol.run_sequence();
    }
    else if ( dim == 3 )
    {
      MPI::RT_Solver::Bragg_double<MPI::Fourier::cft_3d_MPI,3> rtsol( &params );
      rtsol.run_sequence();
    }
    else
    {
