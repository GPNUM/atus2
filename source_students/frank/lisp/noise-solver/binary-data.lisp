...
(defmethod write-bin (out header)
...
(defmethod write-bin (out (data atus2-data))
  (write-bin out (header data))
  (write-bin out (data data)))
...
(defun read-bin-1d (in)
  (let* ((header (read-header in))
         (matrix (make-array (nDimX header)
                             :element-type 'complex :initial-element #C(0.0 0.0))))
    (loop :for i :below (nDimX header)
       :do (setf (aref matrix i) (read-complex64 in)))
    (values header matrix)))
