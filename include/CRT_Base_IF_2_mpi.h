...
#ifndef __class_CRT_Base_IF_2_mpi__
#define __class_CRT_Base_IF_2_mpi__

template <class T, int dim, int no_int_states>
class CRT_Base_IF_2_mpi : public CRT_Base_2_mpi<T,dim,no_int_states>
{
public:
  CRT_Base_IF_2_mpi( ParameterHandler * );
  virtual ~CRT_Base_IF_2_mpi();

  void run_sequence();

protected:
  using CRT_Base_2_mpi<T,dim,no_int_states>::m_header;
  using CRT_Base_2_mpi<T,dim,no_int_states>::m_params;
  using CRT_Base_2_mpi<T,dim,no_int_states>::m_fields;
  using CRT_Base_2_mpi<T,dim,no_int_states>::m_custom_fct;
  using CRT_shared_mpi::m_no_of_pts;
...
template <class T, int dim, int no_int_states>
CRT_Base_IF_2_mpi<T,dim,no_int_states>::CRT_Base_IF_2_mpi( ParameterHandler *params ) : CRT_Base_2_mpi<T,dim,no_int_states>(params)
{
...
template <class T, int dim, int no_int_states>
CRT_Base_IF_2_mpi<T,dim,no_int_states>::~CRT_Base_IF_2_mpi()
{
  if ( m_workspace != nullptr ) fftw_free(m_workspace);
}

template <class T, int dim, int no_int_states>
void CRT_Base_IF_2_mpi<T,dim,no_int_states>::UpdateParams()
{
  for ( int i=0; i<dim; i++)
  {
...
template <class T, int dim, int no_int_states>
void CRT_Base_IF_2_mpi<T,dim,no_int_states>::compute_rabi_integrals()
{
  ...
  fftw_complex *psik2 = m_fields[no_int_states/2]->Get_p2_Data();
  m_fields[0]->ft(-1);
  ...
  MPI_Allreduce( res2, allres2, m, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD );
  ...
  for ( int i=0 ; i<n; i++ )
  {
    tmpvec.push_back(this->m_ar_k*allres[i]);
  }

  for ( int i=0 ; i<m; i++ )
  {
    tmpvec2.push_back(this->m_ar_k*allres2[i]);
  }
...
template <class T, int dim, int no_int_states>
void CRT_Base_IF_2_mpi<T,dim,no_int_states>::Output_rabi_freq_list( string filename, string filename2, const long long Nk )
{
...
template <class T, int dim, int no_int_states>
void CRT_Base_IF_2_mpi<T,dim,no_int_states>::Output_chirps_list( string filename )
{
  ofstream txtfile( filename );
...
template <class T, int dim, int no_int_states>
void CRT_Base_IF_2_mpi<T,dim,no_int_states>::Do_NL_Step_Wrapper ( void *ptr, sequence_item &seq )
{
  CRT_Base_IF_2_mpi<T,dim,no_int_states> *self = static_cast<CRT_Base_IF_2_mpi<T,dim,no_int_states>*>(ptr);
  self->Do_NL_Step();
}

template <class T, int dim, int no_int_states>
void CRT_Base_IF_2_mpi<T,dim,no_int_states>::Do_NL_Step()
{
...
template <class T, int dim, int no_int_states>
void CRT_Base_IF_2_mpi<T,dim,no_int_states>::run_sequence()
{
