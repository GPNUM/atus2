...
#ifndef __class_CRT_Base_mpi__
#define __class_CRT_Base_mpi__

/** Template Class for the propagation in <B>dim</B> dimensions of a BEC with <B>no_int_states</B> internal states in MPI (TODO run sequence)
  *
...
class CRT_Base_mpi : public CRT_shared_mpi
{
public:
  CRT_Base_mpi( ParameterHandler * );
  virtual ~CRT_Base_mpi();
  ...
  void Init_Potential();
  void Setup_Potential( const int, const ptrdiff_t, const double );
...
/** Constructor
  ...
  */
template <class T, int dim, int no_int_states>
CRT_Base_mpi<T,dim,no_int_states>::CRT_Base_mpi( ParameterHandler *params )
{
  m_params = params;
...
/// Destructor
template <class T, int dim, int no_int_states>
CRT_Base_mpi<T,dim,no_int_states>::~CRT_Base_mpi()
{
...
/** Allocate m_fields, m_full_step and m_half_step
  */
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::Allocate()
{
  for ( int i=0; i<no_int_states; i++ )
    m_fields[i] = new T(&m_header);

  m_full_step = (fftw_complex *)fftw_malloc( sizeof(fftw_complex)*m_alloc );
  m_half_step = (fftw_complex *)fftw_malloc( sizeof(fftw_complex)*m_alloc );
}
...
/** The exponential of the kinetic operator in momentum space is calculated according to the operator splitting method.
  ...
  */
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::Init()
{
...
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::Init_Potential()
{
...
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::Setup_Potential( const int comp, const ptrdiff_t index, const double val )
{
...
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::run_sequence()
{
...
/** Load initial wavefunctions from files
  ...
  */
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::LoadFiles()
{
...
/** Wrapper function for Do_FT_Step_full()
  ...
  */
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::Do_FT_Step_full_Wrapper ( void *ptr, sequence_item &seq)
{
  CRT_Base_mpi<T,dim,no_int_states> *self = static_cast<CRT_Base_mpi<T,dim,no_int_states>*>(ptr);
...
/** Wrapper function for Do_Step_half()
  ...
  */
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::Do_FT_Step_half_Wrapper ( void *ptr, sequence_item &seq)
{
  CRT_Base_mpi<T,dim,no_int_states> *self = static_cast<CRT_Base_mpi<T,dim,no_int_states>*>(ptr);
...
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::Do_NL_Step_Wrapper ( void *ptr, sequence_item &seq)
{
  CRT_Base_mpi<T,dim,no_int_states> *self = static_cast<CRT_Base_mpi<T,dim,no_int_states>*>(ptr);
  self->Do_NL_Step();
}
...
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::Do_NL_Step_Wrapper_one ( void * /*ptr*/, sequence_item & /*seq*/ )
{
}
...
/** Computes the full kinetic part
  */
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::Do_FT_Step_full()
{
  //Timer
...
/** Computes half the kinetic part
  */
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::Do_FT_Step_half()
{
  //Timer
...
/** Solves the potential part without any external fields
  */
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::Do_NL_Step()
{
  const double dt = -m_header.dt;
  ...
  for ( int i=0; i<no_int_states; i++ )
    Psi.push_back(m_fields[i]->Get_p2_Data());
...
/** Add a constant momentum to an internal state.
  ...
  */
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::Setup_Momentum( CPoint<dim> px, const int comp )
{
...
/** Calculate the expectation value of the postion of an internal state
  ...
  */
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::Expval_Position( CPoint<dim> &retval, const int comp )
{
  if ( comp<0 || comp>no_int_states ) throw std::string("Error in " + std::string(__func__) + ": comp out of bounds\n");
  ...
  {
    x = m_fields[comp]->Get_x(l);
    den = x*(Psi[l][0]*Psi[l][0]+Psi[l][1]*Psi[l][1]);
    for (int i=0; i<dim; i++ )
      sum[i] += x[i]*den;
  }

  MPI_Allreduce(sum,allsum,dim,MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

  for ( int i=0; i<dim; i++ )
    retval[i] = m_ar*allsum[i];
}
...
/** Calculate the expectation value of the momentum of an internal state
  ...
  */
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::Expval_Momentum( CPoint<dim> &retval, const int comp )
{
  if ( comp<0 || comp>no_int_states ) throw std::string("Error in " + std::string(__func__) + ": comp out of bounds\n");
...
/** Compute the number of particles of an internal state
  ...
  */
template <class T, int dim, int no_int_states>
double CRT_Base_mpi<T,dim,no_int_states>::Get_Particle_Number( const int comp )
{
  if ( comp<0 || comp>no_int_states ) throw std::string("Error in " + std::string(__func__) + ": comp out of bounds\n");
  ...
  MPI_Allreduce(&retval,&tmp,1,MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  return m_ar*tmp;
}

/// Defines Output for << operator for CRT_Base objects
template <class T, int dim, int no_int_states>
void CRT_Base_mpi<T,dim,no_int_states>::Dump_2( ofstream &stream )
{
  stream << m_header.t << "\t";
  //stream << Get_N() << "\t";
}
...
/** \relates CRT_Base
  ...
  */
template<class T, int dim, int no_int_states>
ofstream &operator<<( ofstream &stream, CRT_Base_mpi<T,dim,no_int_states> &obj )
{
  obj.Dump_2(stream);
  return stream;
}
